...
class AdvancedTournament(Tournament):
    ...
    def update_fighter_properties_and_award_coins(self, fighter, flag_defeat=False, flag_rest=False):
        ...
        if len(fighter.history_record) == 3:
            if fighter.history_record[0] == fighter.history_record[1] and fighter.history_record[1] == fighter.history_record[2] and fighter.history_record[0] == 'W' and flag_defeat == False:
            ...
            elif fighter.history_record[0] == fighter.history_record[1] and fighter.history_record[1] == fighter.history_record[2] and fighter.history_record[0] == 'W' and flag_defeat == True:
                AdvancedFighterFile.coins_to_obtain = AdvancedFighterFile.coins_to_obtain * 1.1 * 2
            ...
            elif fighter.history_record[0] == fighter.history_record[1] and fighter.history_record[1] == fighter.history_record[2] and fighter.history_record[0] == 'L':
                AdvancedFighterFile.coins_to_obtain *= 1.1
                AdvancedFighterFile.delta_attack = -2
                AdvancedFighterFile.delta_defense = 2
        ...
        if flag_defeat == True and len(fighter.history_record) <= 3:
            AdvancedFighterFile.coins_to_obtain *= 2
            AdvancedFighterFile.delta_attack = 1
            AdvancedFighterFile.delta_defense = -1
            AdvancedFighterFile.delta_speed = -1
    ...
    def input_fighters(self, team_NO):
        ...
        for fighter_idx in range(4 * (team_NO - 1) + 1, 4 * (team_NO - 1) + 5):
            while True:
                ...
                HP, attack, defense, speed = properties
                if HP + 10 * (attack + defense + speed) <= 500:
                    fighter = AdvancedFighter(fighter_idx, HP, attack, defense, speed)
    ...
    def play_one_round(self):
        ...
        while True:
            ...
            if damage_second is None:
                ...
                if fighter_first == team1_fighter:
            ...
            if winner_info ==  "tie":
                team1_fighter.history_record = []
                team2_fighter.history_record = []
			
            
            team1_fighter.print_info()
            team2_fighter.print_info()
  
            if team1_fighter.check_defeated() == True:
                self.update_fighter_properties_and_award_coins(team1_fighter, False, False)
                self.update_fighter_properties_and_award_coins(team2_fighter, True, False)
            elif team2_fighter.check_defeated() == True:
                self.update_fighter_properties_and_award_coins(team1_fighter, True, False)
                self.update_fighter_properties_and_award_coins(team2_fighter, False, False)
            else:
                self.update_fighter_properties_and_award_coins(team1_fighter, False, False)
                self.update_fighter_properties_and_award_coins(team2_fighter, False, False)
            fight_cnt += 1
